...
import math
from clam.wsi_core.wsi_utils import savePatchIter_bag_hdf5, initialize_hdf5_bag, coord_generator, save_hdf5, sample_indices, screen_coords, isBlackPatch, isWhitePatch,isWhitePatch_S, to_percentiles
from clam.wsi_core.util_classes import isInContourV1, isInContourV2, isInContourV3_Easy, isInContourV3_Hard, Contour_Checking_fn
...
class WholeSlideImage(object):
    ...
    def visWSI(self, vis_level=0, color = (0,255,0), hole_color = (0,0,255), annot_color=(255,0,0), 
                    line_thickness=50, max_size=None, top_left=None, bot_right=None, custom_downsample=1, view_slide_only=False,
    ...
    def process_contours_custom(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        save_path_hdf5 = os.path.join(save_path, str(self.name) + '.h5')
        print("Creating patches for: ", self.name, "...", )
        start_x, start_y, w, h =(
        ...
        for y in range(start_y, stop_y, step_size_y):
            for x in range(start_x, stop_x, step_size_x):
                ...
                if isBlackPatch(np.array(patch_PIL), rgbThresh=50) or isWhitePatch_S(patch_PIL,rgbThresh=220, percentage=0.25):
                        continue
                else:


                    # img = np.array(patch_PIL, dtype="float32")
                    # img = np.array(img, dtype="uint8")
                    # img = np.expand_dims(img, axis=0)
                    # batchImages.append(img)
                    coord_candidates.append([x,y])
        #batchImages = np.vstack(batchImages)

        print('Extracted {} coordinates'.format(len(coord_candidates)))

        if len(coord_candidates) > 1:
            asset_dict = {'coords':   np.vstack(coord_candidates)}
        ...
        save_hdf5(save_path_hdf5, asset_dict, attr_dict, mode='w')
        # image_dir='/data/scratch/DBI/DUDBI/DYNCESYS/OlgaF/ovarian_cancer/images/'
        # os.makedirs(image_dir, exist_ok=True)
        # try:
        #     with h5py.File(os.path.join(image_dir, self.name+'.h5'), "w") as hdf5_file:
        #         hdf5_file.create_dataset('imgs', data=batchImages)
        #         print("Dataset images for slide {} created".format( self.name ))
        # except ValueError:
        #     print("Dataset images for slide {} already exists".format( self.name ))
        #
        # try:
        #     with h5py.File(os.path.join(image_dir, self.name+'.h5'), "a") as hdf5_file:
        #         hdf5_file.create_dataset('coords', data=np.vstack(coord_candidates))
        #         print("Dataset coords for slide {} created".format( self.name ))
        # except ValueError:
        #     print("Dataset coords for slide {} already exists".format( self.name ))
